    ...
    <ul class="main-list-data">
        ...
        <li class="line-data {{ !($key % 2) ? 'line-color2' : 'line-color1' }}">
            <ul class="main-items-list-data">
                ...
                <li class="item-list-date size-item-payment">
                    <button id="button-payment-{{ $line->$collumnId }}" data-id="{{ $line->$collumnId }}"
                        data-payment="{{ $line->payment_status }}" 
                        data-paymentId="{{ $line->payment_id }}" 
                        class="button-payment button-payment-{{$statusMap[$line->payment_status]}}">
                        {{ $statusMap[$line->payment_status] }} 
                    </button>
                </li>
