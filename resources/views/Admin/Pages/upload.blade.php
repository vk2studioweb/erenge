<section id="main-list-data">
    ...
    <form method="post"
    data-url="{{ url('/admin/upload/' . $thisdata->pageConf->pageFatherId . '/' . $thisdata->pageConf->idRegister . '/save') }}"
    action="{{ url('/admin/upload/' . $thisdata->pageConf->pageFatherId . '/'. $thisdata->pageConf->idRegister .'/save') }}"
...
<section id="main-list-images">
   ...
   <h2 class="title-upload">Arquivos Carregados</h2>
    ...
    <ul id="main-list-files" data-csrf="{{ csrf_token() }}">
    @foreach($thisdata->uploadList as $upload)
        <li data-select="false" id="{{ $upload->id_upload }}">
